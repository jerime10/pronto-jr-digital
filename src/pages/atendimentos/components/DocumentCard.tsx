...
import { Download, Share2, File, Trash2 } from 'lucide-react';
import { Document } from '../hooks/useDocuments';
import { WhatsappIcon } from '@/components/lucide';

interface DocumentCardProps {
  document: Document;
  onDownload: (fileUrl: string) => void;
  onShare: (document: Document) => void;
  onDelete?: (documentId: string) => void; // Added delete handler
}
    ...
    <Card className="overflow-hidden hover:shadow-md transition-shadow">
      ...
      <CardContent className="pt-4">
        ...
        <div className="flex gap-2 mt-4">
          ...
          <Button 
            size="sm" 
            className={`flex-1 ${hasPhone ? 'bg-green-600 hover:bg-green-700' : ''}`}
            onClick={() => onShare(document)}
            disabled={!hasPhone}
            title={!hasPhone ? "Paciente sem telefone cadastrado" : undefined}
          >
            <WhatsappIcon />
      ...
      {/* Add Card Footer with Delete Button */}
      {onDelete && (
