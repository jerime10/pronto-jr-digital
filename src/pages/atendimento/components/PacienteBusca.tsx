...
const PacienteBusca: React.FC<PacienteBuscaProps> = ({
  ...
  return <div className="space-y-4">
      ...
      {pacienteSelecionado ? <div className="space-y-4">
          <div className="flex items-center space-x-4 border rounded-lg p-4 bg-card">
            ...
            <Button variant="ghost" size="icon" onClick={onClearPaciente} disabled={disabled} className="hover:bg-destructive/10 hover:text-destructive">
              <X className="h-4 w-4" />
            </Button>
          </div>
          ...
          {mostrarResultadosBusca && <div className="absolute top-full left-0 right-0 z-50 mt-1 bg-popover border rounded-lg shadow-lg max-h-80 overflow-y-auto">
              {isSearchingPacientes && <div className="flex items-center justify-center p-4">
                  <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-primary"></div>
                  <span className="ml-2 text-sm text-muted-foreground">Carregando pacientes...</span>
                </div>}
              
              {!isSearchingPacientes && filteredPacientes.length > 0 && <div className="py-2">
                  {/* Header da lista */}
                  <div className="px-4 py-2 border-b bg-muted/50">
                    <div className="flex items-center text-sm text-muted-foreground">
                      <Users className="h-4 w-4 mr-2" />
                      {buscarPaciente ? `${filteredPacientes.length} paciente(s) encontrado(s)` : `${filteredPacientes.length} paciente(s) cadastrado(s)`}
                    </div>
                  </div>
                  
                  {/* Lista de pacientes */}
                  {filteredPacientes.map(paciente => <div key={paciente.id} className="flex items-center space-x-3 px-4 py-3 hover:bg-accent hover:text-accent-foreground transition-colors cursor-pointer border-b last:border-b-0" onClick={() => onSelectPaciente(paciente)}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {paciente.phone && <p className="text-xs text-muted-foreground">Tel: {paciente.phone}</p>}
                      </div>
                    </div>)}
                </div>}
              
              {!isSearchingPacientes && filteredPacientes.length === 0 && buscarPaciente.length > 0 && <div className="px-4 py-8 text-center">
